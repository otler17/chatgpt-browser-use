...
def login_and_check(browser, email, protected_path, label):
    ...
    if COOKIE_NAME not in cookie_names:
        raise RuntimeError(
            f"{label}: browser did not store {COOKIE_NAME}; cookies={cookies}; url={page.url}"
        )

    response = page.goto(f"{BASE}{protected_path}", wait_until="networkidle", timeout=60000)
    if response is None or response.status != 200:
        raise RuntimeError(f"{label}: protected page failed status={None if response is None else response.status}")
    if "/login" in page.url:
        raise RuntimeError(f"{label}: browser was redirected back to login: {page.url}")

    print(f"BROWSER {label} login passed: cookie={COOKIE_NAME} protected={protected_path} url={page.url}")
    context.close()
