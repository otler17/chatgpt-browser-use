...
SESSION.headers.update(
    {
        "User-Agent": "TunisTourismVideoBuilder/2.0 (contact: github.com/otler17/chatgpt-browser-use)",
...
def download_with_backoff(url: str) -> bytes:
    ...
    for attempt in range(5):
        ...
        try:
            response = SESSION.get(clean_url, timeout=60)
            if response.status_code == 429:
                retry_after = response.headers.get("Retry-After")
                delay = int(retry_after) if retry_after and retry_after.isdigit() else 6 + attempt * 4
            ...
            last_error = exc
    raise RuntimeError(f"Could not download image after retries: {clean_url}: {last_error}")
...
def commons_image(query: str, target_stem: Path) -> tuple[Path, dict]:
    ...
    for page in pages:
        ...
        try:
            time.sleep(1.5)
            path.write_bytes(download_with_backoff(url))
        except Exception as exc:
            print("Skipping image candidate:", exc)
            continue
...
async def edge_segment(text: str, out_path: Path, voice: str) -> None:
    ...
    await edge_tts.Communicate(text=text, voice=voice, rate="+8%").save(str(out_path))
...
async def build() -> None:
    ...
    for item in VIDEOS:
        ...
        for index, query in enumerate(item["shots"], start=1):
            ...
            vf = (
                ...
                "crop=1080:1920,"
                "zoompan=z='min(zoom+0.00023,1.06)':d=300:s=1080x1920:fps=30,"
                "setsar=1"
            )
            run(
                "ffmpeg",
